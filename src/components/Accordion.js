  ...
  return (
    <div className={`${className}  rounded `}>
      <div className={`flex justify-between items-center gap-3 cursor-pointer min-h-[2.5rem] px-3 ${summaryClassName}`} onClick={handleOpen} >
        ...
        {title}
        <div className='grow' />
        <Icon icon='fluent:chevron-down-12-filled' className='transition' width={20} height={20} rotate={open ? 2 : 0} />
      </div>
      <div className={`${open ? "max-h-96 overflow-auto " : "max-h-0 overflow-hidden"} transition `}>
        {children}
      </div>
    </div>
  )
